...
/**
 * Every neuron (16 + 16 + 10) as one InstancedMesh — a single draw call.
 * Per-instance HDR color carries activation brightness into the bloom pass;
 * per-instance scale gives the pop animation.
 */
export class NeuronField {
  ...
  /** Flat instance index of a neuron. Layers: 0=h1, 1=h2, 2=output. */
  ...
  constructor(layout: NetworkLayout) {
    ...
    for (let layer = 0; layer < 3; layer++) {
      ...
      const radius = layer === 2 ? NEURON_RADIUS_OUTPUT : NEURON_RADIUS_HIDDEN;
